...
#		pwd
		...
		if [ $? -eq 1 ];then
		{
			echo "$3/$patch_path/$patch patch Merge Failure @_@"
			exit
		}
		fi
		...
		{
			rm ${patch}
			echo "$3/$patch_path/$patch patch has been merged T_T"
		}
		else
		{
			...
			if [ $? -eq 1 ];then
			{
				echo "$3/$patch_path/$patch patch successful merger ^_^"
			}
			else
			{
				echo "$3/$patch_path/$patch patch Merge Failure @_@"
