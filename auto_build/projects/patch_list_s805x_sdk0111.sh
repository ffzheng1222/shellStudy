...
#注意:给变量赋值时，“=” 前后都不要有空格
function patch_list_s805x_0111()
{
    ROOT_DIR=${PWD} 											#根目录，这里约定为 auto_build.sh 所在目录
    PROJECT_NAME=sdk0111_s805x_ott   							#项目名称，可自行修改，注意不能与其他项目名称相同
    PROJECT_LUNCH_NAME=curie									# sdk编译对应的工程product名
    SDK_MANIFEST=p-amlogic_openlinux-20190111.xml 				# sdk manifest, 即指定公版 sdk 的版本
    SDK_BRANCH=p-amlogic 										# sdk 分支
    SDK_PATCH_LIST=${FUNCNAME}.txt 								# path list，即项目对应的patch list
    SDK_PATCH_BRANCH=master 									# patch 分支
    SDK_SAVE_PATH=${SDK_MANIFEST%.*}	
    SDK_PATH=${ROOT_DIR}/${PROJECT_NAME}/${SDK_SAVE_PATH} 		# 下载的 sdk 存在在此路径
...
add_lunch_combo patch_list_s805x_0111																		# 这里约定 add_lunch_combo 的参数、上面的 function 都跟 SDK_PATCH_LIST 相同
