...
#注意:给变量赋值时，“=” 前后都不要有空格
function patch_list_s805x_hailstorm()
{
    ROOT_DIR=~/workspace # 根目录，这里约定为 auto_build.sh 所在目录
    PROJECT_NAME=hailstorm-2.0.1-s805x #项目名称，可自行修改，注意不能与其他项目名称相同
...
add_lunch_combo patch_list_s805x_hailstorm # 这里约定 add_lunch_combo 的参数、上面的 function 都跟 SDK_PATCH_LIST 相同
